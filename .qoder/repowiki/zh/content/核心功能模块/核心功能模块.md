...
## 简介
本文件面向预算管理系统的各个核心功能模块，提供完整、可操作的技术文档。内容覆盖用户认证与权限管理、预算管理、采购管理、数据分析、系统管理等模块，配套功能流程图、API 接口说明与前端页面展示说明，并补充业务逻辑、数据流转与异常处理机制，帮助开发者与产品人员快速理解与落地。
...
## 结论
本系统通过清晰的路由与守卫机制、完善的认证与权限体系、以及模块化的页面组件，实现了预算管理的核心业务闭环。建议后续在生产环境中接入真实后端 API，完善权限模型与审计日志，并持续优化前端性能与用户体验。
...
- 权限点清单（模块.操作）
  ...
  - 采购：create/read/update/delete/approve/export
  - 审批：read/approve
  - 报表：read/export
