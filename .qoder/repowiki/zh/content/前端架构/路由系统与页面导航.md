...
<cite>
**本文引用的文件**
- [src/router/routes.tsx](file://src/router/routes.tsx)
...
## 更新摘要
**所做更改**
- 更新路由配置文件从 routes.ts 迁移到 routes.tsx，支持 JSX 语法
- 实现 React.lazy 动态导入和代码分割功能
- 路由表扩展至 18 个路由配置，涵盖完整的预算管理功能
- 更新私有路由保护机制，采用 Zustand 状态管理和本地存储双重验证
- 优化权限守卫系统，支持更精细的权限控制
...
B --> D["src/router/routes.tsx<br/>路由配置数组支持JSX"]
D --> E["页面组件懒加载<br/>如 BudgetDetail、PurchaseRequestDetail 等"]
B --> F["src/store/authStore.ts<br/>Zustand 状态存储"]
...
- [src/router/routes.tsx:25-121](file://src/router/routes.tsx#L25-L121)
...
**章节来源**
- [src/router/routes.tsx:1-122](file://src/router/routes.tsx#L1-L122)
...
graph TB
subgraph "路由层"
R1["路由配置数组<br/>src/router/routes.tsx"]
...
```mermaid
graph TB
R["路由配置数组<br/>src/router/routes.tsx"] --> L["懒加载导入页面组件"]
L --> Split["代码分割打包"]
Split --> Load["按需加载页面"]
```

**图表来源**
- [src/router/routes.tsx:4-22](file://src/router/routes.tsx#L4-L22)
...
```mermaid
graph TB
RT["路由配置数组(routes.tsx)"] --> APP["根组件(App.tsx)"]
