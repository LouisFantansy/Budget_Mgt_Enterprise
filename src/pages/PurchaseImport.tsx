...
import type { ImportResult, ImportError } from '../api/modules/import-export.api'
...
export function PurchaseImport() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await importExportApi.importPurchaseOrders(file, (percent) => {
      ...
      if (result.success) {
        if (result.failedRows > 0) {
          ...
          setSuccess(`成功导入 ${result.successRows} 条采购订单`)
        }
      } else {
  ...
  const handleDownloadTemplate = async () => {
    try {
      await importExportApi.downloadTemplate('purchase-orders')
    } catch (err: any) {
